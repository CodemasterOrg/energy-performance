...
const Table: FC = () => {
  ...
  useEffect(() => {
    fetch(
      "https://api.energidataservice.dk/dataset/PowerSystemRightNow?limit=500"
    )
      .then((res) => {
      ...
      .then((json) => {
        ...
        const grouped = raw.reduce<Record<string, HourlyAggregatedData>>(
          (acc, curr) => {
            const hourKey = new Date(curr.Minutes1DK)
              .toISOString()
              .substring(0, 13); // eks: '2025-07-16T12'
            if (!acc[hourKey]) {
              acc[hourKey] = {
                hour: hourKey,
                OffshoreWindPower: 0,
        ...
        const hourlyData = Object.values(grouped).sort((a, b) =>
